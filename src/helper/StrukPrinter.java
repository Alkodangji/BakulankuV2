...
public final class StrukPrinter {
    ...
    public static void printBBM(
        ...
        List<String> lines = baseHeader("STRUK BBM", nomorTransaksi, tanggal);
        lines.add("BBM      : " + safe(namaBbm));
        lines.add("Liter    : " + formatQty(liter));
        lines.add("Harga    : " + RupiahFormat.format(hargaJual));
        lines.add("Total    : " + RupiahFormat.format(total));
        lines.add("Metode   : " + safe(metodePembayaran));
        lines.add("Diterima : " + RupiahFormat.format(diterima));
        lines.add("Kembali  : " + RupiahFormat.format(kembalian));
    ...
    public static void printKeuangan(
            String nomorTransaksi,
            Date tanggal,
            String jenis,
            String kategori,
        ...
        List<String> lines = baseHeader("STRUK KEUANGAN", nomorTransaksi, tanggal);
        lines.add("Jenis    : " + safe(jenis));
        lines.add("Kategori : " + safe(kategori));
        if (akunAsal != null && !akunAsal.trim().isEmpty()) {
            lines.add("Akun Asal: " + akunAsal);
        }
        if (akunTujuan != null && !akunTujuan.trim().isEmpty()) {
            lines.add("Akun Tujuan: " + akunTujuan);
        }
        lines.add("Nominal  : " + RupiahFormat.format(nominal));
        if (catatan != null && !catatan.trim().isEmpty()) {
            lines.add("Catatan  : " + catatan);
        }
        printLines(lines);
    }
